# NVIDIA Dynamo Blueprint Catalog - v0.8.1 Validation Results
...
✅ **v0.8.1 Comprehensive Testing Complete** (100% Pass Rate for Testable Configurations)
...
| Tier | Purpose | Who It's For |
|------|---------|-------------|
| **core** | First-success examples — one per backend (vllm, sglang, trtllm) in aggregated mode, plus vllm disaggregated and router | New users getting started |
| **standard** | Production features — observability, KVBM, multi-replica, multinode, multimodal, backend-parity disaggregated/router variants | Users building production deployments |
| **advanced** | Profiling workflows — DGDR planner, LoRA/DynamoModel, high-performance TRT-LLM | Advanced users optimizing performance |
| **experimental** | In-progress or blocked examples needing investigation | Developers and testers |
| **model-showcase** | Specific model + hardware combinations | Users deploying specific models on specific instances |
...
## Model Showcase (5 Model Families · 32 Entries)
...
# 20B - Disaggregated ✅
./deploy.sh showcase-gptoss-20b-sglang-dgd

# 20B - Router ✅
./deploy.sh showcase-gptoss-20b-sglang-router
```
...
```bash
# 70B - Aggregated ✅
./deploy.sh showcase-llama-3.3-70b-sglang-agg

# 70B - Disaggregated ✅
./deploy.sh showcase-llama-3.3-70b-sglang-dgd
```
...
```bash
# 70B - Aggregated ✅
./deploy.sh showcase-deepseek-70b-sglang-agg
```
...
**Issue:** DGDR profiler has initialization bug in v0.8.1
**Impact:** DGDR blueprints in advanced tier untestable
**Workaround:** None currently, awaiting v0.7.2 or v0.8.1 fix
...
- Test results are tracked in the `test-results/` directory and this catalog README
- See the [Validation Status](#validation-status) section above for the latest results
...
- All test results reflect v0.8.1 validation on g6e.12xlarge infrastructure
