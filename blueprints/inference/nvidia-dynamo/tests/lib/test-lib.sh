...
validate_model_list() {
    ...
    if [ "$model_count" -eq 0 ]; then
        # Retry — model may still be loading (large models take 10-30 min)
        info "No models registered yet. Waiting for model to load (retrying up to 5 min)..."
        local retry=0
        while [ "$model_count" -eq 0 ] && [ $retry -lt 10 ]; do
            sleep 30
            retry=$((retry + 1))
            models_response=$(curl -s "${base_url}/v1/models" --max-time 15 2>/dev/null)
            model_count=$(echo "$models_response" | jq '.data | length' 2>/dev/null || echo 0)
            [ "$model_count" = "null" ] && model_count=0
            info "  Retry ${retry}/10... models registered: ${model_count}"
        done
        if [ "$model_count" -eq 0 ]; then
            error "❌ No models registered after 5 min of retries."
            return 1
        fi
    fi
...
discover_model() {
    ...
    # Status messages go to stderr so stdout only contains the clean model name
    # (this function is typically called inside $() command substitution)
    info "Discovering available models..." >&2

    local models_response=$(curl -s "${base_url}/v1/models" --max-time 15 2>/dev/null)

    if [ -z "$models_response" ] || echo "$models_response" | grep -qi "error"; then
        warn "Could not retrieve models list, using fallback: ${fallback_model}" >&2
        echo "$fallback_model"
        return 1
    fi
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        if [ -n "$discovered_model" ] && [ "$discovered_model" != "null" ]; then
            success "Discovered model: ${discovered_model}" >&2
    ...
    # Fallback
    warn "Using fallback model: ${fallback_model}" >&2
    echo "$fallback_model"
    return 1
}
...
test_chat_completion() {
    ...
    # Detect multimodal models — they require list-format content
    ...
    if echo "$model" | grep -qiE "llava|-vl-|pixtral|internvl"; then
        is_multimodal=true
    fi
    ...
    if [ "$is_multimodal" = "true" ]; then
        # Detect video vs image multimodal
        local is_video=false
        if echo "$model" | grep -qiE "video|next-video"; then
            is_video=true
        fi

        if [ "$is_video" = "true" ]; then
            # Video models require video_url content type
            local test_video="https://test-videos.co.uk/vids/bigbuckbunny/mp4/h264/720/Big_Buck_Bunny_720_10s_1MB.mp4"
            chat_payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"${prompt}\"}, {\"type\": \"video_url\", \"video_url\": {\"url\": \"${test_video}\"}}]}], \"max_tokens\": ${max_tokens}, \"temperature\": ${temperature}}"
        else
            # Image multimodal models require image_url content type
            local test_image="https://picsum.photos/id/237/200/300"
            chat_payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"${prompt}\"}, {\"type\": \"image_url\", \"image_url\": {\"url\": \"${test_image}\"}}]}], \"max_tokens\": ${max_tokens}, \"temperature\": ${temperature}}"
        fi
    else
        chat_payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": \"${prompt}\"}], \"max_tokens\": ${max_tokens}, \"temperature\": ${temperature}}"
    fi
    ...
    if [ -n "$response" ] && ! echo "$response" | grep -qi "error"; then
        ...
        echo "$response" | jq -r '.choices[0].message.content // .choices[0].message.reasoning_content' 2>/dev/null || echo "$response" | head -3
        return 0
    else
