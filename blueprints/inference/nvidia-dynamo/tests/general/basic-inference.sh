...
#---------------------------------------------------------------
# Warmup Request (handles TRT-LLM JIT compilation cold start)
#---------------------------------------------------------------
run_warmup_request() {
    section "Warmup Request"

    local base_url="http://localhost:${LOCAL_PORT}"
    local model=$(discover_model "$base_url" "Qwen/Qwen3-0.6B")

    info "Sending warmup request (first inference may trigger JIT compilation)..."

    # Detect multimodal — use list-format content
    # Pattern: -VL- (delimited) catches Qwen2.5-VL-7B, Qwen3-VL-8B etc.
    # Does NOT match text-only models like Qwen3-30B-A3B or Qwen3-8B.
    local is_multimodal=false
    if echo "$model" | grep -qiE "llava|-vl-|pixtral|internvl"; then is_multimodal=true; fi
    # Note: HAS_MULTIMODAL env var no longer overrides model-name detection
    # to prevent false positives on text-only vLLM deployments (P0-1 fix)

    local warmup_timeout=120
    local is_video=false
    echo "$model" | grep -qiE "video|next-video" && is_video=true
    local test_image="https://picsum.photos/id/237/200/300"
    local test_video="https://test-videos.co.uk/vids/bigbuckbunny/mp4/h264/720/Big_Buck_Bunny_720_10s_1MB.mp4"
    local payload
    if [ "$is_multimodal" = "true" ] && [ "$is_video" = "true" ]; then
        payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"Say hello.\"}, {\"type\": \"video_url\", \"video_url\": {\"url\": \"${test_video}\"}}]}], \"max_tokens\": 10, \"temperature\": 0.1}"
    elif [ "$is_multimodal" = "true" ]; then
        payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"Say hello.\"}, {\"type\": \"image_url\", \"image_url\": {\"url\": \"${test_image}\"}}]}], \"max_tokens\": 10, \"temperature\": 0.1}"
    else
        payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": \"Say hello.\"}], \"max_tokens\": 10, \"temperature\": 0.1}"
    fi

    local saved_timeout=$REQUEST_TIMEOUT
    REQUEST_TIMEOUT=$warmup_timeout
    local response=$(api_call POST "/v1/chat/completions" "$payload" 2>/dev/null || echo "")
    REQUEST_TIMEOUT=$saved_timeout

    if [ -n "$response" ] && ! echo "$response" | grep -qi "error"; then
        success "Warmup complete — engine is ready for inference tests"
        record_test_result "warmup" "passed"
    else
        warn "Warmup request did not succeed (engine may still be compiling). Continuing tests..."
        record_test_result "warmup" "warning"
    fi
}
...
run_sequential_tests() {
    ...
    for i in "${!prompts[@]}"; do
        ...
        local payload
        if [ "$is_multimodal" = "true" ] && [ "$is_video" = "true" ]; then
            payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"${prompt}\"}, {\"type\": \"video_url\", \"video_url\": {\"url\": \"${test_video}\"}}]}], \"max_tokens\": 50, \"temperature\": 0.1}"
        elif [ "$is_multimodal" = "true" ]; then
            payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"${prompt}\"}, {\"type\": \"image_url\", \"image_url\": {\"url\": \"${test_image}\"}}]}], \"max_tokens\": 50, \"temperature\": 0.1}"
        else
            payload="{\"model\": \"${model}\", \"messages\": [{\"role\": \"user\", \"content\": \"${prompt}\"}], \"max_tokens\": 50, \"temperature\": 0.1}"
        fi
