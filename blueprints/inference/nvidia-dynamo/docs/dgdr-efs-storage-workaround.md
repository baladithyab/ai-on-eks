# DGDR EFS Storage Workaround for v0.8.0

## Problem

The Dynamo v0.8.0 Kubernetes operator uses `emptyDir` volumes for DGDR profiling output, which consumes ephemeral storage on the node. For large models like Qwen2.5-Coder-32B that require 3-4+ hours of profiling, this can lead to ephemeral storage exhaustion and pod eviction.
...
The v0.8.0 operator hardcodes `emptyDir` volumes in the profiler job:

```go
// v0.8.0 code in dynamographdeploymentrequest_controller.go
...
The `outputPVC` field that allows specifying a PVC was added in later commits (after v0.8.0):
...
When upgrading to a Dynamo operator version > v0.8.0, you can use the native `outputPVC` field:
...
spec:
  profilingConfig:
    outputPVC: dynamo-pvc  # Native PVC support (v0.8.0+)
    # ... rest of config
```
