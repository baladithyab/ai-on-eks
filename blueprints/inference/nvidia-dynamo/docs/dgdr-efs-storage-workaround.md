# DGDR EFS Storage Workaround for v0.8.1

## Problem

The Dynamo v0.8.1 Kubernetes operator uses `emptyDir` volumes for DGDR profiling output, which consumes ephemeral storage on the node. For large models like Qwen2.5-Coder-32B that require 3-4+ hours of profiling, this can lead to ephemeral storage exhaustion and pod eviction.
...
The v0.8.1 operator hardcodes `emptyDir` volumes in the profiler job:

```go
// v0.8.1 code in dynamographdeploymentrequest_controller.go
...
The `outputPVC` field that allows specifying a PVC was added in later commits (after v0.8.1):
...
2. ~~The patch script was previously at `scripts/patch-profiler-job-pvc.sh` but has been removed as it is obsolete for v0.8.1. Use the native `outputPVC` field instead (see below).~~

### Usage (Legacy — script removed)

> **Note:** The `patch-profiler-job-pvc.sh` script has been removed. This section is retained for historical reference only. For v0.8.1+, use the native `outputPVC` field in DGDR specs instead.

```bash
# Apply the DGDR with native outputPVC (v0.8.1+)
kubectl apply -f features/dgdr-planner/vllm-dgdr-qwen-coder-32b.yaml
```
...
## Native PVC Support (v0.8.1+)

Dynamo v0.8.1 natively supports the `outputPVC` field in DGDR specs, making the manual patching workaround above a **fallback** option. If you are running the v0.8.1 operator, use the native field directly:
...
spec:
  profilingConfig:
    outputPVC: dynamo-pvc  # Native PVC support (v0.8.1+)
    # ... rest of config
```

> **Note:** The manual patching script (`patch-profiler-job-pvc.sh`) has been removed as it is obsolete for v0.8.1+. Use the native `outputPVC` field in your DGDR specs instead.
