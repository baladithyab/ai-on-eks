...
#
# Comprehensive testing framework for Dynamo smoke and workflow coverage with:
# - Catalog-backed tier discovery for core/standard/advanced
...
CATALOG_FILE="$BLUEPRINTS_DIR/catalog/catalog.yaml"
...
# Read catalog entries as id|tier rows
catalog_entries() {
    local file="${1:-$CATALOG_FILE}"
    [[ -f "$file" ]] || return 1
...
get_catalog_blueprints_for_tier() {
    local requested_tier="$1"
    catalog_entries "$CATALOG_FILE" | awk -F'|' -v requested_tier="$requested_tier" '$2 == requested_tier { print $1 }'
}

# Get blueprints for a smoke/workflow tier
get_blueprints_for_tier() {
    local tier="$1"
    local blueprints=()
    local catalog_blueprints=()
    ...
    case "$tier" in
        all)
            mapfile -t blueprints < <(
                get_blueprints_for_tier "core"
                get_blueprints_for_tier "standard"
                get_blueprints_for_tier "advanced"
            )
            ;;
        core)
            blueprints=("hello-world")
            mapfile -t catalog_blueprints < <(get_catalog_blueprints_for_tier "core")
            blueprints+=("${catalog_blueprints[@]}")
            ;;
        standard|advanced)
            mapfile -t blueprints < <(get_catalog_blueprints_for_tier "$tier")
            ;;
        *)
...
main() {
    ...
    local blueprints=()
    mapfile -t blueprints < <(get_blueprints_for_tier "$TIER")
...
Tiers:
  core      - hello-world + catalog core (small golden path)
  standard  - catalog standard (common follow-on patterns)
  advanced  - catalog advanced (specialized workflows)
