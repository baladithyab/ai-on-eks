...
verify_pod_otel_config() {
    ...
    if [ -n "$DEPLOYMENT" ]; then
        label_selector="nvidia.com/dynamo-namespace=${DEPLOYMENT}"
    else
        label_selector="nvidia.com/dynamo-namespace"
    fi
...
verify_trace_backend() {
    ...
    local tempo=$(kubectl get deployment tempo -n "${MONITORING_NAMESPACE}" --ignore-not-found 2>/dev/null || \
                  kubectl get statefulset tempo -n "${MONITORING_NAMESPACE}" --ignore-not-found 2>/dev/null || echo "")
    
    if [ -n "$tempo" ]; then
        log_pass "Tempo backend found in namespace ${MONITORING_NAMESPACE}"
        
        # Check Tempo health
        local tempo_pod=$(kubectl get pods -n "${MONITORING_NAMESPACE}" -l app=tempo --no-headers 2>/dev/null | head -1 | awk '{print $1}')
        if [ -n "$tempo_pod" ]; then
            local tempo_ready=$(kubectl get pod "$tempo_pod" -n "${MONITORING_NAMESPACE}" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null)
        ...
        # Check Tempo service
        local tempo_svc=$(kubectl get svc tempo -n "${MONITORING_NAMESPACE}" -o jsonpath='{.spec.clusterIP}' 2>/dev/null || echo "")
        if [ -n "$tempo_svc" ]; then
            log_pass "Tempo service available at: tempo.${MONITORING_NAMESPACE}.svc.cluster.local"
        fi
...
verify_trace_connectivity() {
    ...
    if [ "$CHECK_BACKEND" = true ]; then
        ...
        local tempo_result=$(kubectl exec -n "${NAMESPACE}" "$test_pod" -- \
            sh -c "timeout 5 nc -zv tempo.${MONITORING_NAMESPACE}.svc.cluster.local 4317 2>&1" 2>/dev/null || echo "failed")
...
print_summary() {
    ...
    echo "  Backend (Tempo):    $(kubectl get deployment tempo -n ${MONITORING_NAMESPACE} --ignore-not-found &>/dev/null && echo "✓ Deployed" || echo "- Not checked")"
