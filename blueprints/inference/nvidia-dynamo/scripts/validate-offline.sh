...
check_links() {
    ...
    start_check "Blueprint docs relative link checks"

    # Scope to the blueprint's own docs — website/docs is a repo-wide concern
    # and should not cause blueprint-specific validation failures.
    local docs_dirs=()
    [ -d "${BLUEPRINT_DIR}/docs" ] && docs_dirs+=("${BLUEPRINT_DIR}/docs")
    [ -d "${BLUEPRINT_DIR}" ] && docs_dirs+=("${BLUEPRINT_DIR}")

    # Collect markdown files only from blueprint-scoped directories
    local md_files=()
    ...
    if [ ${#md_files[@]} -eq 0 ]; then
        log_warn "No markdown files found in blueprint directory for link checks"
        return 0
    fi
...
# Strip trailing :line_number references (e.g., catalog.yaml:1)
    ...
    with open(path, "r", encoding="utf-8") as fh:
        ...
        for lineno, line in enumerate(fh, start=1):
            ...
            for match in link_pattern.findall(line):
                ...
                if not target:
                    continue
                # Strip :line_number references (e.g., file.yaml:1)
                target = line_ref_pattern.sub("", target)
                if not target:
                    continue
                candidate = os.path.normpath(os.path.join(os.path.dirname(path), target))
                # Skip links that resolve outside the blueprint directory
                # (those belong to repo-wide link validators, not blueprint-scoped)
...
PY
    then
        log_pass "Blueprint docs relative links look valid"
    else
        log_warn "Blueprint docs have broken relative links (see above)"
    fi
}
